...
function buildQcRow(srNo, inspection, project, item) {
  ...
  return [
    ...
    photoHyperlink                       // AC: Product Photos (Google Drive Link)
  ];
}


// ============================================================
// HELPER: Ensure header row in QC Log sheet (29 Columns)
// ============================================================
function ensureHeaderRow(sheet) {
  ...
  // Overwrite Row 1 with latest 29-column headers
  ...
  sheet.setColumnWidth(29, 220); // Product Photos (Drive Link)
}
...
function formatNewRows(sheet, fromRow, toRow) {
  ...
  // Product Photos Drive Link (AC = col 29)
  for (var r = fromRow; r <= toRow; r++) {
    var photoCell = sheet.getRange(r, 29);
    photoCell.setFontColor("#1d4ed8").setFontWeight("bold").setHorizontalAlignment("center");
  }
